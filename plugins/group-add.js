...
let handler = async (m, { conn, text, participants }) => {
    ...
    let users = (await Promise.all(
        text.split(',')
            ...
            .map(async v => [
                v,
                await conn.isOnWhatsApp(v + '@s.whatsapp.net') // perbaikan pada fungsi name
            ])
    )).filter(v => v[1][0]?.exists).map(v => v[0] + '@c.us')

    // Tambahkan penanganan untuk kasus pengguna hanya memasukkan perintah
    if (users.length === 0) {
        return m.reply('Mana nomornya? Awali dengan 628×××')
    }
