...
let handler = async (m, { isPrems }) => {
  ...
  if (new Date - global.db.data.users[m.sender].udahklaim < 86400000) throw `Kamu Sudah Bilang Hari Ini\nTunggu Selama ${msToTime(time - new Date())} Lagi`
        global.db.data.users[m.sender].koin += koin
        global.db.data.users[m.sender].limit += limit
        conn.reply(m.chat, `Karna Kamu Bilang Arss ganteng\nKamu Mendapatkan:\n\n+${limit} Limit\n+${koin} Koin`, m)
        global.db.data.users[m.sender].udahklaim = new Date * 1
    }
handler.customPrefix =
	/^(Arss ganteng|Arss ganteng|Arss ganteng)$/i;
